...
export function useFluidSimulation(config: Partial<SimConfig> = {}) {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const animationRef = useRef<number>(0)
  const configRef = useRef<SimConfig>({ ...defaultConfig, ...config })
  ...
  const startSimulation = useCallback(() => {
    ...
    const blurProgram = new GLProgram(
      gl,
      blurVertexShaderCompiled,
      compileShader(gl, gl.FRAGMENT_SHADER, blurShader)
    )
    ...
    const colorProgram = new GLProgram(
      gl,
      baseVertexShaderCompiled,
      compileShader(gl, gl.FRAGMENT_SHADER, colorShader)
    )
    const checkerboardProgram = new GLProgram(
      gl,
      baseVertexShaderCompiled,
      compileShader(gl, gl.FRAGMENT_SHADER, checkerboardShader)
    )
    const bloomPrefilterProgram = new GLProgram(
      gl,
      baseVertexShaderCompiled,
      compileShader(gl, gl.FRAGMENT_SHADER, bloomPrefilterShader)
    )
    const bloomBlurProgram = new GLProgram(
      gl,
      baseVertexShaderCompiled,
      compileShader(gl, gl.FRAGMENT_SHADER, bloomBlurShader)
    )
    const bloomFinalProgram = new GLProgram(
      gl,
      baseVertexShaderCompiled,
      compileShader(gl, gl.FRAGMENT_SHADER, bloomFinalShader)
    )
    const sunraysMaskProgram = new GLProgram(
      gl,
      baseVertexShaderCompiled,
      compileShader(gl, gl.FRAGMENT_SHADER, sunraysMaskShader)
    )
    const sunraysProgram = new GLProgram(
      gl,
      baseVertexShaderCompiled,
      compileShader(gl, gl.FRAGMENT_SHADER, sunraysShader)
    )
    ...
    // Dithering texture
    const ditheringTexture = createDitheringTexture(gl)
    ...
    function createDitheringTexture(gl: GL): DitheringTexture {
      const texture = gl.createTexture()!
      gl.bindTexture(gl.TEXTURE_2D, texture)
    ...
    function blit(gl: GL, target: FBO | null, clear = false) {
      if (target == null) {
    ...
    function initBloomFramebuffers() {
      const res = getResolution(configRef.current.BLOOM_RESOLUTION)
      const texType = ext.halfFloatTexType
      const rgba = ext.formatRGBA!
      const filtering = ext.supportLinearFiltering ? gl!.LINEAR : gl!.NEAREST
    ...
    function initSunraysFramebuffers() {
      const res = getResolution(configRef.current.SUNRAYS_RESOLUTION)
      const texType = ext.halfFloatTexType
      const r = ext.formatR!
      const filtering = ext.supportLinearFiltering ? gl!.LINEAR : gl!.NEAREST
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.code === 'KeyP') {
        configRef.current.PAUSED = !configRef.current.PAUSED
      }
      if (e.key === ' ') {
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: Color) {
      ...
      gl!.uniform1f(
        ...
        correctRadius(configRef.current.SPLAT_RADIUS / 100.0, canvas!.width / canvas!.height)
      )
    ...
    function render(target: FBO | null) {
      ...
      if (!configRef.current.TRANSPARENT) {
        drawColor(target, normalizeColor(configRef.current.BACK_COLOR))
      }
      if (target == null && configRef.current.TRANSPARENT) {
...
export function FluidHeroBackground({ className = '', config }: FluidHeroBackgroundProps) {
  const canvasRef = useFluidSimulation(config)
